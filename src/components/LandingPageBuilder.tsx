...
export default function LandingPageBuilder({
  ...
  const webinar = webinarSlug
    ? webinars.find((w: any) => w.slug === webinarSlug)
    : null;
  ...
  const handleFileUpload = async (blockId: string, key: string, file: File) => {
    try {
      ...
      const storageRef = ref(
        storage,
        `landing-pages/${Date.now()}_${file.name}`,
      );
  ...
  return (
    <div className="flex flex-col h-full bg-slate-100 dark:bg-zinc-950 overflow-hidden rounded-bl-3xl rounded-br-3xl">
      ...
      {webinarSlug && (
        <header className="h-14 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-4">
            <button
              onClick={onBack}
              className="p-2 -ml-2 text-slate-500 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-white rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"
            >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-56 bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800 flex flex-col shrink-0 overflow-y-auto hidden md:flex">
        ...
        {/* Center Canvas */}
        <div
          className="flex-1 overflow-auto p-4 md:p-8 flex justify-center bg-slate-50 dark:bg-black/20"
          onClick={() => setSelectedBlockId(null)}
        >
          <div className="w-full min-w-[320px] max-w-4xl bg-white dark:bg-zinc-950 min-h-full rounded-2xl shadow-xl overflow-hidden border border-slate-200 dark:border-zinc-800">
            {blocks.length === 0 ? (
        ...
        <div className="w-72 bg-white dark:bg-zinc-900 border-l border-slate-200 dark:border-zinc-800 shrink-0 overflow-y-auto flex flex-col">
          {selectedBlock ? (
            <>
                  ...
                  if (Array.isArray(selectedBlock.props[key])) {
                    return (
                      <div key={key} className="space-y-3">
                        ...
                        {selectedBlock.props[key].map(
                          (item: string, i: number) => (
                            <div key={i} className="flex gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newArr = selectedBlock.props[
                                    key
                        ...
                        <button
                          onClick={() => {
                            updateBlockProp(selectedBlock.id, key, [
                      ...
                      {key === "subheadline" ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            ...
                            />
                            <button
                              disabled={uploadingProp === key}
                              className="w-full px-3 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30 rounded-lg text-sm font-medium flex items-center justify-center gap-2 cursor-pointer pointer-events-none"
                            >
                              {uploadingProp === key ? (
      ...
      {selectedBlockId === block.id && (
        <div className="absolute -top-10 right-0 bg-indigo-500 text-white flex items-center rounded-t-lg shadow-lg overflow-hidden h-9">
          ...
          <button
            ...
            className="w-9 h-full flex items-center justify-center hover:bg-red-500 transition-colors"
          >
            <Trash2 className="w-4 h-4" />
          </button>
        </div>
  ...
  switch (type) {
    case "hero":
      return {
        ...
        subheadline:
          "Add a compelling subheadline that elaborates on the value provided.",
    ...
    case "instructor":
      return {
        name: "Host Name",
        bio: "Expert in the field with years of experience.",
        role: "CEO & Founder",
      };
    default:
      return {};
...
export function RenderBlock({
    ...
    return (
      <div className="py-24 px-8 bg-black text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto space-y-8 mt-12 mb-8">
          ...
          <h1 className="text-4xl sm:text-5xl font-display font-extrabold text-white leading-tight tracking-tight break-words text-balance">
            {props.headline}
          </h1>
          ...
          <div className="pt-10 text-center max-w-sm mx-auto">
            <button
              ...
              className="w-full py-5 px-8 bg-white text-black rounded-2xl font-bold text-lg hover:bg-zinc-200 transition-all shadow-[0_0_40px_rgba(255,255,255,0.15)] active:scale-95 group flex items-center justify-center gap-2"
            >
              {props.buttonText}
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </button>
            <p className="text-zinc-600 mt-4 text-sm font-medium flex items-center justify-center gap-1.5">
              <ShieldCheck className="w-4 h-4" /> Secure, 1-Click Registration
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid gap-4">
            {props.features?.map((feat: string, i: number) => (
              <div
                key={i}
                className="flex gap-4 p-4 border border-slate-100 dark:border-zinc-800 rounded-xl bg-slate-50 dark:bg-zinc-900/50"
              >
  ...
  if (type === "video") {
    const isMp4 =
      ...
      props.url?.includes(".webm") ||
      props.url?.includes("firebasestorage");
    const isYoutube =
      ...
      <div className="py-16 px-8 bg-white dark:bg-zinc-950">
        <div className="max-w-3xl mx-auto flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="text-center md:text-left space-y-2">
            ...
            <p className="text-slate-600 dark:text-zinc-400 max-w-lg mt-4">
              {props.bio}
            </p>
          </div>
        </div>
